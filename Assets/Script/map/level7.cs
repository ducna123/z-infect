...
public class level7 : MonoBehaviour {
    ...
    void Start () {
        ...
        myBody4 = brick3.GetComponent<Rigidbody2D>();
        myBody5 = brick4.GetComponent<Rigidbody2D>();
    }
        ...
        if(Time.timeScale != 0 && win == false)
        {
            ...
            if(change == false)
            {
                if (player.GetComponent<playerController>().isChange == true)
                {
                    ...
                    zb.transform.localScale = new Vector3(49, 49, 49);
                    change = true;
                }
            }
    ...
    void _check()
    {
        for (int i = 0; i < 2; i++)
        {
            if (lv7[i].gameObject.tag == "zombie")
            {
                dem++;
            }
        }
